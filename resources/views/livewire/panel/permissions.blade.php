                ...
                <div class="card-body border-0 pt-6">
                    <form wire:submit="search" class="position-relative my-1 w-100">
                        <div class="row">
                            ...
                            <div class="col-sm-5 pb-3">
                                <select class="form-select form-select-solid ps-5" id="field_filter_groups" wire:model="search_group">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5">
                            <thead>
                                <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                                    ...
                                    <th class="w-100px">Grupo</th>
                                    <th class="text-end min-w-100px" width="100">Ações</th>
                                </tr>
                            </thead>
                            <tbody class="text-gray-600 fw-bold">
                                @foreach ($permissions as $permission)
                                <tr @if($last_id_update == $permission->id) class="bg-light-success" @endif wire:key="list_item_{{$permission->id}}">
                                    <td class="d-flex align-items-center">
                                        <div class="d-flex flex-column">
                                            <a href="{{route('panel.permissions.edit',['permission'=>$permission->id])}}" class="text-gray-800 text-hover-primary mb-1 d-inline-block" wire:navigate>
                                                {{$permission->label}} 
                                            </a>
                                            <i class="small">{{$permission->name}}</i>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="badge badge-light fw-bolder"> {{$group_list[$permission->group_name]}} </div>
                                    </td>
                                    <td class="text-end">
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown position-absolute menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">                                       
                                            <div class="menu-item px-3">
                                                <a href="{{route('panel.permissions.edit',['permission'=>$permission->id])}}" class="menu-link px-3" wire:key="edit_{{$permission->id}}" wire:navigate>
                                                    Editar
                                                </a>
                                            </div>
                                            <div class="separator my-2"></div>
                                            <div class="menu-item px-3">
                                                <a href="javascript:void(0);" class="text-danger menu-link px-3" wire:click="delete_item({{$permission->id}})" wire:confirm="Tem certeza que deseja excluír este registro? Esta ação não poderá ser desfeita!" wire:key="status_{{$permission->id}}">
